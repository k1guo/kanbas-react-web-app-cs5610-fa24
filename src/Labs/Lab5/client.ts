...
// /lab5/todos/:id/completed/:completedc

export const updateDescription = async(todo: any) =>{
  const response = await axios.get(`${TODOS_API}/todos/${todo.id}/description/${todo.description}`);
  return response.data;
};
...
// /lab5/todos/:id/description/:description
  ...
  export const updateCompleted = async (completed: boolean) => {
    const response = await axios.get(`/lab5/assignment/completed/${completed}`);
   return (response.data);
  };
